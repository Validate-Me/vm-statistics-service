...
async function main2(companyName,tableData,fileNames,fileDict) {
  let message=('<head><style>a:link {color: skyblue;text-decoration:None} table, th, td {word-break: break-all;border: 1px solid black;border-collapse: collapse;text-align:centre;border-spacing: 5px;padding:7px;}</style></head>'+
    ...
    '<img  style="width:250px;" src="https://www.linkpicture.com/q/logo_280.png">'+
  ...
  for(let issuerName of Object.keys(companyName)) {
   ...
   for(let person of Object.keys(userNames)){
    ...
    message += (
        `<tr style="word-wrap:break-word; "><td >${person}</td>`+
         ...
         //console.log(data_exist)
          ...
          // console.log(nameoffile)
            ...
            if (data_user.length===1){
              ...
              // console.log(fileNames[[companyuserid]])
...
var userData={}
    for (let companyid of Object.keys(dic)) {
      var me=[]
  const uNames = Object.keys(dic[companyid]);
  
  for (let i=0;i<uNames.length;i=i+1){
   if (userData[uNames[i]]){
    userData[uNames[i]]=userData[uNames[i]]+dic[companyid][uNames[i]]

   }
   else{
    userData[uNames[i]]=dic[companyid][uNames[i]]
   }
...
for (let companyid of Object.keys(dic)) 
{
  ...
  {
    // console.log(dic[companyid])
    const vNames = Object.keys(dic[companyid]);
    
    const uNames = Object.keys(tableData[docpercompany[companyid]]);
    // console.log(uNames[0])
    for(let i=0;i<uNames.length;i++){
      if(dic[companyid][uNames[i]]){
        tableData[docpercompany[companyid]][uNames[i]]=tableData[docpercompany[companyid]][uNames[i]]+dic[companyid][uNames[i]]
        
      }
      else{
        tableData[docpercompany[companyid]][vNames[0]]=userData[vNames[0]]
      }
    }
    // console.log(dic[companyid])
    // console.log(tableData[docpercompany[companyid]][uNames[0]])
    // tableData[docpercompany[companyid]][uNames[0]]=dic[companyid]


  // {console.log(tableData[docpercompany[companyid]])
    for (let username of Object.keys(tableData[docpercompany[companyid]])){
      // console.log(tableData[docpercompany[companyid]])
      // console.log(tableData[docpercompany[companyid]][username])
      // console.log(dic[companyid])
